...
public class BlockHeraldry extends BlockContainer {
	...
	public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer, int par6, float par7, float par8, float par9) {
		...
		if(tile != null && tile instanceof TileEntityBanner && !par1World.isRemote) {
			...
			if(holding && stack.itemID == ModItems.heraldryItem.itemID && stack.getItemDamage() == 0) {
				if(banner.locked) {
	...
	public int idDropped(int par1, Random par2Random, int par3) {
		return ModItems.heraldryItem.itemID;
	}
	...
	public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z) {
		return new ItemStack(ModItems.heraldryItem, 1, isHanging(world.getBlockMetadata(x, y, z)) ? 2 : 1);
	}
